...
class CAT048 {
  ...
  async setTargetReportDescriptor(buffer, numTarget) {
    var octet1
    var octet2
    var octet3
    if (numTarget === 1) {
    ...
    if (octet1 != undefined) {
      ...
      this.targetReportDescriptor.SIM =
        octet1[3] === "1" ? "Simulated target report" : "Actual target report"
      this.targetReportDescriptor.RDP =
        octet1[4] === "1"
          ? "Report from RDP Chain 2"
          : "Report from RDP Chain 1"
      this.targetReportDescriptor.SPI =
        octet1[5] === "1" ? "Special Position Identification" : "Absence of SPI"
      this.targetReportDescriptor.RAB =
        octet1[6] === "1"
    ...
    if (octet2 != undefined) {
      this.targetReportDescriptor.TST =
        octet2[0] === "1" ? "Test target report" : "Real target report"
      this.targetReportDescriptor.ERR =
        octet2[1] === "1" ? "Extended Range present" : "No Extended Range"
      this.targetReportDescriptor.XPP =
        octet2[2] === "1" ? "X-Pulse present" : "No X-Pulse present"
      this.targetReportDescriptor.ME =
        octet2[3] === "1" ? "Military emergency" : "No military emergency"
      this.targetReportDescriptor.MI =
        octet2[4] === "1"
    ...
    if (octet3 != undefined) {
      this.targetReportDescriptor.ADSB_EP =
        octet3[0] === "1" ? "ADSB populated" : "ADSB not populated"
      this.targetReportDescriptor.ADSB_VAL =
        octet3[1] === "1"
          ? "On-Site ADS-B Information available"
          : "On-Site ADS-B Information not available"
      this.targetReportDescriptor.SCN_EP =
        octet3[2] === "1" ? "SCN populated" : "SCN not populated"
      this.targetReportDescriptor.SCN_VAL =
        octet3[3] === "1"
          ? "Surveillance Cluster Network Information available"
          : "Surveillance Cluster Network Information not available"
      this.targetReportDescriptor.PAI_EP =
        octet3[4] === "1" ? "PAI populated" : "PAI not populated"
      this.targetReportDescriptor.PAI_VAL =
        octet3[5] === "1"
          ? "Passive Acquisition Interface Information available"
          : "Passive Acquisition Interface Information not available"
      this.targetReportDescriptor.SPARE =
        octet3[6] === "1" ? "Spare Bit, not set to 0" : "Spare Bit, set to 0"
      //var FX = octet3[7] === '1' ? 'Extension into next extent' : 'End of Data Item';
    }
  }

  async setMeasuredPositionPolarCoordinates(buffer) {
    var RHO = parseInt(
      buffer[0].toString(2).padStart(8, "0") +
        buffer[1].toString(2).padStart(8, "0"),
      2
    )
    const rho = RHO / 256
    ...
    var THETA = parseInt(
      buffer[2].toString(2).padStart(8, "0") +
        buffer[3].toString(2).padStart(8, "0"),
      2
    )
  ...
  async setLatitudeLongitude() {
    ...
    this.calculatedPositionLLACoordinates.lat =
  ...
  async setCalculatedPositionCartesianCoordinates(buffer) {
    const XcomponentBits =
      buffer[0].toString(2).padStart(8, "0") +
      buffer[1].toString(2).padStart(8, "0")
    const YcomponentBits =
      buffer[2].toString(2).padStart(8, "0") +
      buffer[3].toString(2).padStart(8, "0")

    function twoComplementOfChainBits(chainBits) {
  ...
  async setMode3ACodeOctalRepresentation(buffer) {
    var octet1 = buffer[0].toString(2).padStart(8, "0")
    var octet2 = buffer[1].toString(2).padStart(8, "0")

    this.mode3ACodeOctalRepresentation.V =
      octet1[0] === "1" ? "Code not validated" : "Code validated"
    this.mode3ACodeOctalRepresentation.G =
      octet1[1] === "1" ? "Garbled code" : "Default"
    this.mode3ACodeOctalRepresentation.L =
      octet1[2] === "1"
        ? "Mode-3/A code not extracted during the last scan"
        : "Mode-3/A code derived from the reply of the transponder"
    var bit13 =
      octet1[3] === "1" ? "Spare bit not set to 0" : "Spare bit set to 0"
  ...
  async setFlightLevelBinaryRepresentation(buffer) {
    var octet1 = buffer[0].toString(2).padStart(8, "0")
    var octet2 = buffer[1].toString(2).padStart(8, "0")

    this.flightLevelBinaryRepresentation.V =
      octet1[0] === "1" ? "Code not validated" : "Code validated"
    this.flightLevelBinaryRepresentation.G =
      octet1[1] === "1" ? "Garbled code" : "Default"
  ...
  async setHeightMeasuredBy3DRadar(buffer) {
    var octet1 = buffer[0].toString(2).padStart(8, "0")
    var octet2 = buffer[1].toString(2).padStart(8, "0")
  ...
  async setRadarPlotCharacteristics(buffer, subfield) {
    if (subfield === 0) {
      var SRL = buffer
      this.radarPlotCharacteristics.SRL =
        (parseInt(SRL, 2) * (360 / Math.pow(2, 13))).toFixed(5) + " dg"
    } else if (subfield === 1) {
      var SRR = buffer
      ...
      var PRL = buffer
      this.radarPlotCharacteristics.PRL =
        (parseInt(PRL, 2) * (360 / Math.pow(2, 13))).toFixed(3) + " dg"
    } else if (subfield === 4) {
      var PAM = buffer
      this.radarPlotCharacteristics.PAM = parseInt(PAM, 2) + " dBm"
    } else if (subfield === 5) {
      var RPD = buffer
      this.radarPlotCharacteristics.RPD =
        (parseInt(RPD, 2) / 256).toFixed(4) + " nmi"
    } else if (subfield === 6) {
      var APD = buffer
      this.radarPlotCharacteristics.APD =
        (parseInt(APD, 2) * (360 / Math.pow(2, 14))).toFixed(4) + " dg"
    }
  }

  async setTimeOfDay(buffer) {
    ...
    function secondsToMs(seconds) {
      ...
      return `${formatoHora(hours)}:${formatoHora(minutes)}:${formatoHora(
        remainingSeconds
      )}:${miliseconds}`
    }
  }

  async setTrackNumber(buffer) {
    let trackNumberBits = ""
  ...
  async setTrackStatus(buffer) {
    ...
    charBitsTrackStatus[3] === "0"
      ? (this.trackStatus.DOU = "Normal confidence")
      : (this.trackStatus.DOU = "Low confidence in plot to track association")
    charBitsTrackStatus[4] === "0"
      ? (this.trackStatus.MAH = "No horizontal man.sensed")
      : (this.trackStatus.MAH = "Horizontal man. sensed")
  ...
  async setTrackStatus2(buffer) {
    const bitsTrackStatus = buffer[0].toString(2).padStart(8, "0")
    const charBitsTrackStatus = bitsTrackStatus.split("")
    charBitsTrackStatus[0] === "0"
      ? (this.trackStatus.TRE = "Track still alive")
      : (this.trackStatus.TRE = "End of track lifetime")
    charBitsTrackStatus[0] === "0"
      ? (this.trackStatus.GHO = "True target track")
      : (this.trackStatus.GHO = "Ghost target track")
    charBitsTrackStatus[0] === "0"
      ? (this.trackStatus.SUP = "no")
      : (this.trackStatus.SUP = "yes")
    charBitsTrackStatus[0] === "0"
      ? (this.trackStatus.TCC =
          "Tracking performed in so-called 'Radar Plane', i.e. neither slant range correction nor stereographical projection was applied")
      : (this.trackStatus.TCC =
  ...
  async setCalculatedTrackVelocityPolarCoordinates(buffer) {
    var RHO = parseInt(
      buffer[0].toString(2).padStart(8, "0") +
        buffer[1].toString(2).padStart(8, "0"),
      2
    )
    this.calculatedTrackVelocityPolarCoordinates.rho = Number(
      (RHO * 0.22).toFixed(4)
    )
    var THETA = parseInt(
      buffer[2].toString(2).padStart(8, "0") +
    ...
    this.calculatedTrackVelocityPolarCoordinates.theta = Number(
      (THETA * (360 / Math.pow(2, 16))).toFixed(4)
    )
  }

  async setAircraftAddress(buffer) {
  ...
  async binaryToHex(binaryString) {
    const binToHexTable = {
      "0000": "0",
      "0001": "1",
      "0010": "2",
  ...
  async setCommunicationsACASCapabilityFlightStatus(buffer) {
    var octet1 = buffer[0].toString(2).padStart(8, "0")
    var octet2 = buffer[1].toString(2).padStart(8, "0")
    ...
    switch (COM_int) {
      case 0:
        this.communicationsACASCapabilityFlightStatus.COM =
    ...
    switch (STAT_int) {
      case 0:
        this.communicationsACASCapabilityFlightStatus.STAT =
    ...
    this.communicationsACASCapabilityFlightStatus.SI =
      octet1[7] === "1" ? "II-Code Capable" : "SI-Code Capable"
    this.communicationsACASCapabilityFlightStatus.MSSC =
      octet2[0] === "1" ? "Yes" : "No"
    this.communicationsACASCapabilityFlightStatus.ARC =
      octet2[1] === "1" ? "25 ft resolution" : "100 ft resolution"
    this.communicationsACASCapabilityFlightStatus.AIC =
      octet2[2] === "1" ? "Yes" : "No"
  ...
  async setAircraftIdentification(buffer) {
    let aircraftIdentificationBytes = ""
    for (let i = 0; i < buffer.length; i += 1) {
      aircraftIdentificationBytes =
        aircraftIdentificationBytes + buffer[i].toString(2).padStart(8, "0")
    }
    this.decodeAircraftIdentification(aircraftIdentificationBytes)
  }
  async decodeAircraftIdentification(chainBits) {
    const binToIDTable = {
      "000000": "",
      "000010": "P",
      "000001": "",
  ...
  async setModeBDS(bufferRegister, chainBitsData) {
    ...
    this.BDSRegisterData.modeS =
      this.BDSRegisterData.modeS + "BDS: " + decimalBDS1 + "," + decimalBDS2
    const decodeModeBDS4 = chainBits => {
      ...
      if (parseInt(targetAltStatus, 2) === 1) {
        this.BDSRegisterData.targetAltStatus =
          "Source information deliberately provided"
      } else {
    ...
    const decodeModeBDS5 = chainBits => {
      ...
      if (RASstatus == "1") {
        ...
        if (angle != 90 && angle != -90) {
          this.BDSRegisterData.RollAngle =
            (sign === "1" ? -1 : 1) * parseFloat(angle.toFixed(3))
        }
      }
      ...
      if (TTAstatus == "1") {
        ...
        if (angle != 180 && angle != 180) {
          this.BDSRegisterData.TrueTrackAngle =
            (sign === "1" ? -1 : 1) * parseFloat(angle.toFixed(3))
        }
      }
      ...
      if (TARstatus == "1") {
        ...
        this.BDSRegisterData.TrackAngleRate =
          (sign === "1" ? -1 : 1) * parseFloat(angle.toFixed(3))
      }
    ...
    const decodeModeBDS6 = chainBits => {
      ...
      if (BARstatus == "1") {
        ...
        this.BDSRegisterData.BAR =
          (sign === "1" ? -1 : 1) * parseFloat(ft.toFixed(3))
      }
...
function binaryToDecimalComplementOfTwo(binaryStr) {
  if (binaryStr[0] === "1") {
    const invertedBinaryStr = binaryStr
      .split("")
      .map(bit => (bit === "0" ? "1" : "0"))
      .join("")
